...
| `src/adapter.ts` | `graphFromConfig` — `@textplus/core` GameConfig → neutral `StoryGraph` (parallel links deduplicated; movement-phrased labels become edge directions; gated links become `conditional` edges, drawn dashed in the dev view) |
...
| `src/gamemap.ts` | `renderGameMap(engine, target, {travel?})` — the **player-facing** dungeon map: fog-of-war (only visited rooms), you-are-here marker, fast-travel to visited rooms, per-world layer; positions come from laying out the full world graph so rooms never rearrange as the fog lifts |
