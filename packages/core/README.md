...
|---|---|
| `engine.ts` | `GameEngine` via the `createGame(config)` factory: `goToSituation`/`followLink`, `getAvailableLinks`, quality get/set/mutate (clamped to declared bounds), `reset()`, situation history (`hasSituationBeenVisited`, `getSituationHistory`), event subscriptions (`onSituationChange`, `onQualityChange`, plus legacy `…Changed` aliases), save-state assembly (`getSaveState`/`loadState`, version-checked), `checkCondition`, `validate()`, and getters for situations/qualities/definitions |
